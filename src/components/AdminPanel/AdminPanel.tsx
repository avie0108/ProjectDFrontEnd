...
import { NewChatroomPanel } from "./NewPanels/NewPanels";
import * as Data from "../../Data";
import "./AdminPanel.scss";
import { Chatroom, User } from "../ChatPanel/Sockets/Sockets";
import { ChatroomDetailPanel } from "./DetailPanels/DetailPanels";
			...
			<PopUp ref={this.PopUpRef} Header="Administrator Paneel" canClose={true}>
				<div className="admin-div">
					...
					<div className="user-div">
						<h2>Gebruikers:</h2>
						<div className="users">
							{Data.getUsers().map(x=> <UserItem User={x}/>)}
						</div>
						<button onClick={()=>{this.PopUpRef.current?.Hide(); this.NewChatroomRef.current?.Show();}}>Nieuwe gebruiker</button>
					</div>
				</div>
			</PopUp>
...
class UserItem extends React.Component<UserItemProps,{}>{
	render(){
		return <div className="user">
			<div className="header"><h3 className="user-name">{this.props.User.Username}</h3><button><FontAwesomeIcon icon={faEdit} /></button></div>
